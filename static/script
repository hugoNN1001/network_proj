...
async function loadStudents() {
  // Fetch student list
  const response = await fetch("/api/studentlist"); //  fetch("http://127.0.0.1:5000/api/studentlist");
  const data = await response.json();

  // Fetch average
  const avgResponse = await fetch("/api/average"); // fetch("http://127.0.0.1:5000/api/average");
